import { _ as _defineProperty, F as FileOrBufferOrStringSchema$1, B as BasisPointsSchema$1, Q as QuantitySchema, D as DEFAULT_QUERY_ALL_COUNT, a as CommonNFTInput } from './QueryParams-24720dd7.esm.js';
import { PublicKey, Keypair, clusterApiUrl, Connection } from '@solana/web3.js';
import { z } from 'zod';
import { guestIdentity, amount, SOL, isKeypairSigner, keypairIdentity, isIdentitySigner, walletAdapterIdentity, token, findMetadataPda, toBigNumber, sol, TransactionBuilder, getSignerHistogram, GmaBuilder, toMetadata, toMetadataAccount, findEditionMarkerPda, toDateTime, resolveClusterFromConnection, Pda, TokenProgram, Metaplex } from '@metaplex-foundation/js';
import { createCreateMetadataAccountV2Instruction, EditionMarker, TokenStandard } from '@metaplex-foundation/mpl-token-metadata';
...
class NFTHelper {
  ...
  toNFTMetadataResolved(mint, owner, fullModel) {
    return {
      metadata: {
...
class NFTDrop {
  ...
  async lazyMint(metadatas, options) {
    ...
    const block = await this.metaplex.connection.getLatestBlockhash();
    const txns = batches.map((batch, i) => this.metaplex.candyMachines().builders().insertItems({
      candyMachine,
      ...
      feePayer: this.metaplex.identity().publicKey,
      lastValidBlockHeight: block.lastValidBlockHeight
    }).setFeePayer(this.metaplex.identity()).toTransaction()); // make the connected wallet sign both candyMachine + registry transactions

    const signedTx = await this.metaplex.identity().signAllTransactions(txns); // send the signed transactions

    const signatures = await Promise.all(signedTx.map(tx => this.metaplex.connection.sendRawTransaction(tx.serialize()))); // wait for confirmations in parallel
